...
bool ProtocolBase<Impl>::encodeValue(PyObject* value, TType type, PyObject* typeargs) {
  ...
  case T_UUID: {
    ...
    if (PyBytes_Check(value)) {
      ...
      nval.reset(PyObject_GetAttrString(value, "bytes"));
      if (!nval) {
        return false;
      }
      if (!PyBytes_Check(nval.get())) {
        PyErr_SetString(PyExc_TypeError, "UUID.bytes must be bytes");
        return false;
      }
    }
    ...
    writeBuffer(PyBytes_AS_STRING(nval.get()), 16);
    return true;
  }
...
PyObject* ProtocolBase<Impl>::decodeValue(TType type, PyObject* typeargs) {
  ...
  case T_UUID: {
    ...
    ScopedPyObject bytes_obj(PyBytes_FromStringAndSize(buf, 16));
    if (!bytes_obj) {
      return nullptr;
    }
    ScopedPyObject uuid_module(PyImport_ImportModule("uuid"));
    if (!uuid_module) {
      return nullptr;
    }
    ScopedPyObject uuid_cls(PyObject_GetAttrString(uuid_module.get(), "UUID"));
    if (!uuid_cls) {
      return nullptr;
    }
    ...
    ScopedPyObject kwargs(Py_BuildValue("{sO}", "bytes", bytes_obj.get()));
    if (!kwargs) {
      return nullptr;
    }
    return PyObject_Call(uuid_cls.get(), args.get(), kwargs.get());
  }
