...
TFrozenBaseT = TypeVar("TFrozenBaseT", bound="TFrozenBase")


class TBase(object):
    __slots__ = ()
    thrift_spec: Any = None
...
class TFrozenBase(TBase):
    ...
    def __hash__(self, *args: Any) -> int:
        return hash(self.__class__) ^ hash(self.__slots__)

    @classmethod
    def read(cls: type[TFrozenBaseT], iprot: TProtocolBase) -> TFrozenBaseT:
        if (iprot._fast_decode is not None and
                isinstance(iprot.trans, TTransport.CReadableTransport) and
