...
class TBinaryProtocol(TProtocolBase):
    ...
    VERSION_MASK: Final[int] = -65536

    # VERSION_1 = 0x80010000
    VERSION_1: Final[int] = -2147418112

    TYPE_MASK: Final[int] = 0x000000ff

    def __init__(self, trans: TTransportBase, strictRead: bool = False, strictWrite: bool = True, **kwargs: object) -> None:
        TProtocolBase.__init__(self, trans)
        self.strictRead = strictRead
        self.strictWrite = strictWrite
        self.string_length_limit: int | None = kwargs.get('string_length_limit', None)
        self.container_length_limit: int | None = kwargs.get('container_length_limit', None)
    ...
    def writeUuid(self, uuid_val: UUID) -> None:
        self.trans.write(uuid_val.bytes)
    ...
    def readUuid(self) -> UUID:
        return UUID(bytes=self.trans.readAll(16))


class TBinaryProtocolFactory(TProtocolFactory):
    def __init__(self, strictRead: bool = False, strictWrite: bool = True, **kwargs: object) -> None:
        self.strictRead = strictRead
        self.strictWrite = strictWrite
        self.string_length_limit: int | None = kwargs.get('string_length_limit', None)
        self.container_length_limit: int | None = kwargs.get('container_length_limit', None)
...
class TBinaryProtocolAccelerated(TBinaryProtocol):
    ...
    def __init__(self, trans: TTransportBase, strictRead: bool = False, strictWrite: bool = True,
                 fallback: bool = True, **kwargs: object) -> None:
        super(TBinaryProtocolAccelerated, self).__init__(trans, strictRead, strictWrite, **kwargs)
        try:
            import importlib
            fastbinary = importlib.import_module("thrift.protocol.fastbinary")
        except ImportError:
            if not fallback:
                raise
        else:
