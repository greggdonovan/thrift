...
class _HeaderTransport(Protocol):
    sequence_id: int
    protocol_id: int
    ...
    def readFrame(self, size: int) -> None:
        ...
...
class THeaderProtocol(TProtocolBase):
    ...
    def __init__(self, transport: TTransportBase, allowed_client_types: Iterable[int],
                 default_protocol: int = THeaderSubprotocolID.BINARY) -> None:
        ...
        self._header_trans = cast(_HeaderTransport, transport)
        self._set_protocol()

    def get_headers(self) -> dict[bytes, bytes]:
        return self._header_trans.get_headers()

    def set_header(self, key: bytes, value: bytes) -> None:
        self._header_trans.set_header(key, value)

    def clear_headers(self) -> None:
        self._header_trans.clear_headers()

    def add_transform(self, transform_id: int) -> None:
        self._header_trans.add_transform(transform_id)

    def writeMessageBegin(self, name: str, ttype: int, seqid: int) -> None:
        self._header_trans.sequence_id = seqid
    ...
    def writeUuid(self, uuid_val: UUID) -> None:
        return self._protocol.writeUuid(uuid_val)

    def _set_protocol(self) -> None:
        try:
            protocol_cls = PROTOCOLS_BY_ID[self._header_trans.protocol_id]
        except KeyError:
        ...
        self._protocol = protocol_cls(self._header_trans)
    ...
    def readMessageBegin(self) -> tuple[str, int, int]:
        try:
            self._header_trans.readFrame(0)
            self._set_protocol()
        except TApplicationException as exc:
            self._protocol.writeMessageBegin("", TMessageType.EXCEPTION, 0)
            exc.write(self._protocol)
            self._protocol.writeMessageEnd()
            self._header_trans.flush()
...
class THeaderProtocolFactory(TProtocolFactory):
    def __init__(
        self,
        allowed_client_types: Iterable[int] = (THeaderClientType.HEADERS,),
