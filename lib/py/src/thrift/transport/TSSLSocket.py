...
class TSSLBase(object):
    ...
    def _validate_tls_version(self, ssl_version: Any) -> None:
    ...
    def _enforce_minimum_tls(self, context: ssl.SSLContext) -> None:
        if self._minimum_tls_version is None:
            return
    ...
    def _validate_context_tls(self, context: ssl.SSLContext) -> None:
        if self._minimum_tls_version is None:
            return
        if hasattr(context, 'minimum_version'):
            if context.minimum_version < self._minimum_tls_version:
    ...
    def _init_context(self, ssl_version: Any) -> ssl.SSLContext:
  ...
  """
    ...
    def _deprecated_arg(self, args: tuple[Any, ...], kwargs: dict[str, Any], pos: int, key: str) -> Any:
        if len(args) <= pos:
            return
        real_pos = pos + 3
        warnings.warn(
    ...
    def __init__(self, server_side: bool, host: str | None, ssl_opts: Any) -> None:
...
class TSSLSocket(TSocket.TSocket, TSSLBase):
    ...
    def close(self) -> None:
        if not self.handle:
            return
        try:
            ssl_sock = cast(ssl.SSLSocket, self.handle)
            ssl_sock.settimeout(0.001)
            self.handle = ssl_sock.unwrap()
    ...
    def _do_open(self, family: int, socktype: int) -> ssl.SSLSocket:
    ...
    def open(self) -> None:
        ...
        if self._should_verify:
            ...
            self.peercert = cast(ssl.SSLSocket, self.handle).getpeercert()
            try:
...
class TSSLServerSocket(TSocket.TServerSocket, TSSLBase):
    ...
    def accept(self) -> TSocket.TSocket | None:
        ...
        if self._should_verify:
            ...
            try:
                self._validate_callback(peercert, addr[0])
            except Exception:
