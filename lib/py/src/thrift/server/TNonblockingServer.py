...
class Worker(threading.Thread):
    ...
    def __init__(self, queue: queue.Queue[Any]) -> None:
...
def locked(func: Callable[..., Any]) -> Callable[..., Any]:
    ...
    def nested(self, *args: Any, **kwargs: Any) -> Any:
        self.lock.acquire()
        try:
...
class TNonblockingServer(object):
    """Non-blocking server."""
    _read: socket.socket | None
    _write: socket.socket | None
    poll: select.poll | None
    socket: TSocket.TServerSocket

    def __init__(self,
                 processor: Any,
                 lsocket: Any,
                 inputProtocolFactory: Any = None,
                 outputProtocolFactory: Any = None,
                 threads: int = 10) -> None:
    ...
    def _select(self) -> tuple[list[int], list[int], list[int], bool]:
        ...
        if self.socket.handle is None or self._read is None:
            return [], [], [], False
        ...
        if remaining:
            ...
            rlist, wlist, xlist = select.select(readable, writable, readable)
            return [int(fd) for fd in rlist], [int(fd) for fd in wlist], [int(fd) for fd in xlist], True

    def _poll_select(self) -> tuple[list[int], list[int], list[int], bool]:
        """Does poll on open connections, if available."""
        if self.poll is None or self.socket.handle is None or self._read is None:
            return [], [], [], False
        remaining = []
    ...
    def handle(self) -> None:
        ...
        if self._read is None or self.socket.handle is None:
            return
